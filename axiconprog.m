...
%       ph2(m,n)=0;
        %phL(m,n)=1*exp(1i*(2*pi/lambda)*(sqrt(f^2+x(m,n).^2+y(m,n).^2)-f));
        wave(m,n)=phAX(m,n);
        %wavef(m,n)=(ph1(m,n)+ph2(m,n)).*phf(m,n);
        phia(m,n)=angle(wave(m,n))*180/pi;
        %phif(m)=angle(phf(m))*180/pi;
        %phiaf(m)=angle(wavef(m))*180/pi;
        %ampf(m)=abs(phf(m));
        ampa(m,n)=abs(wave(m,n));
        %ampaf(m)=abs(wavef(m))/0.5301;% 0.5279=max(ampaf)     
...
%         end
        for i3=1:b1
                ...
                elseif (phid_difa(m,n,i3)<=phase_limit+5)    && (amp_difa(m,n,i3)<=amplitude_limit+0.04)
                    ...
                    phisaf(m,n,1)=phasefinal(i3,1);
                    ampsaf(m,n,1)= ampfinal(i3,1);
                elseif (phid_difa(m,n,i3)<=phase_limit+7.5)    && (amp_difa(m,n,i3)<=amplitude_limit+.06)
                    alphasizeaf(m,n,1)=alphafinal(i3,1);
                    risizeaf(m,n,1)=rifinal(i3,1);
                    phisaf(m,n,1)=phasefinal(i3,1);
                    ampsaf(m,n,1)= ampfinal(i3,1);
...
fid = fopen('D:\axicon\Axicone,differenlines\alphas.txt','wt');
...
risizeaf=reshape(risizeaf,1681,1);
 fid = fopen('D:\axicon\Axicone,differenlines\ris.txt','wt');
